...
export const calculateThroughput = (packets, intervalSeconds = 1) => {
  ...
  return Object.entries(grouped).map(([interval, pkts]) => {
    const totalBytes = pkts.reduce((sum, p) => sum + (parseInt(p.Length) || 0), 0);
...
export const detectSecurityIssues = (packets) => {
  ...
  const failedConnections = packets.filter(p => 
    p.Info && p.Info.includes('Retransmission')
  );
