        ...
        {filteredConnections.length === 0 ? (
          <div className="bg-gray-50 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-600">
              No connections found matching your filter
            </p>
          </div>
        ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="bg-gray-50 rounded p-3">
                  ...
                  <div className="flex gap-2 mt-2">
                    <span
                      className={`px-2 py-1 rounded text-xs ${connection.syn ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}
                    >
                      SYN
                    </span>
                    <span
                      className={`px-2 py-1 rounded text-xs ${connection.synAck ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}
                    >
                      SYN-ACK
                    </span>
                    <span
                      className={`px-2 py-1 rounded text-xs ${connection.ack ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}
                    >
                      ACK
                    </span>
                  </div>
                ...
                <div className="bg-gray-50 rounded p-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`font-medium ${connection.established ? "text-green-600" : "text-gray-400"}`}
                      >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`font-medium ${connection.fin ? "text-blue-600" : "text-gray-400"}`}
                      >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`font-medium ${connection.closed ? "text-green-600" : "text-gray-400"}`}
                      >
                    ...
                    {connection.packets.length > 0 && (
                      <>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-mono text-xs">
                            {parseFloat(
                              connection.packets[connection.packets.length - 1]
                                .Time,
                            ).toFixed(3)}
                            s
                          </span>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <div className="space-y-2 text-sm text-blue-800">
          <div className="flex items-start gap-2">
            ...
            <div>
              <span className="font-medium">3-Way Handshake:</span> SYN →
              SYN-ACK → ACK establishes a connection
            </div>
          </div>
          <div className="flex items-start gap-2">
            <Activity className="w-4 h-4 mt-0.5 flex-shrink-0" />
            <div>
              <span className="font-medium">Active Connection:</span>{" "}
              Successfully established but not yet closed
            </div>
          </div>
          <div className="flex items-start gap-2">
            <Clock className="w-4 h-4 mt-0.5 flex-shrink-0" />
            <div>
              <span className="font-medium">Connection Termination:</span> FIN →
              ACK closes the connection gracefully
            </div>
          </div>
          <div className="flex items-start gap-2">
            <XCircle className="w-4 h-4 mt-0.5 flex-shrink-0" />
            <div>
              <span className="font-medium">Failed Connection:</span> Handshake
              incomplete, connection never established
            </div>
          </div>
        </div>
      </div>
