...
function App() {
  ...
  const compareLogFiles = useCallback((logs) => {
    const comparison = {
      ...
      summary: {
        totalPackets: logs.reduce(
          (sum, log) => sum + log.statistics.totalPackets,
          0,
        ),
        uniqueProtocols: new Set(
          logs.flatMap((log) => Object.keys(log.statistics.protocols)),
        ),
        uniqueSources: new Set(
          logs.flatMap((log) => Object.keys(log.statistics.sources)),
        ),
        uniqueDestinations: new Set(
          logs.flatMap((log) => Object.keys(log.statistics.destinations)),
        ),
      },
      differences: [],
      similarities: [],
    ...
    const allSources = logs.map(
      (log) => new Set(Object.keys(log.statistics.sources)),
    );
    const commonSources = allSources.reduce(
    ...
    if (maxSize / minSize > 2) {
      comparison.inconsistencies.push({
        ...
        severity: "MEDIUM",
        description:
          "Significant variation in average packet sizes across logs",
    ...
    logs.forEach((log, idx) => {
      const duration =
        log.report.overview.timeSpan.end - log.report.overview.timeSpan.start;
      if (duration < 1) {
        comparison.inconsistencies.push({
          type: "duration",
          severity: "LOW",
  ...
  const currentLogData =
    selectedLog !== null ? analyzedLogs[selectedLog] : null;
          ...
          <div className="max-w-6xl w-full">
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
              <button
                ...
                disabled={
                  isProcessing ||
                  uploadedFiles.filter((f) => f !== null).length === 0
                }
            ...
            {isProcessing && (
              <div className="bg-white rounded-lg shadow-lg p-6 text-center">
                ...
                <p className="text-lg font-medium text-gray-700">
                  Processing {uploadedFiles.filter((f) => f !== null).length}{" "}
                  file(s)...
                </p>
